...
export default async function handler(request: Request) {
  ...
  try {
        // Reconstruct the absolute URL from Vercel headers
      // Vercel serverless functions receive relative paths, but URL() requires absolute URLs
      const host = request.headers.get('host') || request.headers.get('x-forwarded-host') || 'localhost:3000';
        const protocol = request.headers.get('x-forwarded-proto') || 'https';

      // Handle case where request.url might already be absolute or just a path
      const requestPath = request.url.startsWith('http') 
