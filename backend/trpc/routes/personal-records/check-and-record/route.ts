...
export const checkAndRecordPRProcedure = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    if (fetchError && fetchError.code !== 'PGRST116') {
      console.error('[checkAndRecordPR] Error fetching existing PR:', fetchError);
      throw new TRPCError({
    ...
    if (isNewPR) {
      ...
      if (upsertError) {
        console.error('[checkAndRecordPR] Error recording PR:', upsertError);
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
      ...
      console.log('[checkAndRecordPR] New PR recorded for exercise:', input.exerciseId);
      return {
        isNewPR: true,
    ...
    console.log('[checkAndRecordPR] No new PR for exercise:', input.exerciseId);
    return {
      isNewPR: false,
