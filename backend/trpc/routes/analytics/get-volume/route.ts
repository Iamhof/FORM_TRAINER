...
export const getVolumeProcedure = protectedProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    console.log('[getVolume] Fetching volume for period:', period, 'user:', ctx.userId);
    ...
    const totalVolume = currentAnalytics?.reduce(
      (sum, record) => sum + (record.total_volume || 0),
      0
    ) || 0;

    const uniqueDates = new Set(currentAnalytics?.map(record => record.date) || []);
