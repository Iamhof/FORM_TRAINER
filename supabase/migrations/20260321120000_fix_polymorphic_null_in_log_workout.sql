-- Fix: replace to_jsonb(null) with 'null'::jsonb in log_workout_transaction
-- PostgreSQL cannot determine the polymorphic type of an untyped NULL passed
-- to to_jsonb(anyelement), causing error 42804 at runtime.
--
-- Also ensures analytics.updated_at column exists (idempotent).

-- 1. Add updated_at to analytics if missing
ALTER TABLE analytics ADD COLUMN IF NOT EXISTS updated_at timestamptz DEFAULT now();

-- 2. Backfill existing rows (idempotent — only sets rows that are still NULL)
UPDATE analytics SET updated_at = created_at WHERE updated_at IS NULL;

-- 3. Drop and recreate with 'null'::jsonb fix
DROP FUNCTION IF EXISTS public.log_workout_transaction(uuid, uuid, text, integer, integer, jsonb, timestamptz);
