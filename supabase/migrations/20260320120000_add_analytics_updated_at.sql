-- Fix: add missing updated_at column to analytics table
-- log_workout_transaction references this column but it was never created

-- 1. Add the column
ALTER TABLE analytics ADD COLUMN updated_at timestamptz DEFAULT now();

-- 2. Backfill existing rows
UPDATE analytics SET updated_at = created_at WHERE updated_at IS NULL;

-- 3. Drop and re-create log_workout_transaction so PostgreSQL
--    re-validates the function body against the now-complete schema.
--    The function body is identical to 20260316000001.
